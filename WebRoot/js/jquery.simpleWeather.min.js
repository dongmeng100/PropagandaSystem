	...
	t.extend({
		simpleWeather:function(i){
			...
			if(""!==i.location){
				...
				if(""===i.woeid)
					return i.error("位置无效，无法获取天气信息！"),!1;//Could not retrieve weather due to an invalid location.
			...
			t.ajax({  
            	...
            	success: function (t) {
            		if(null!==t&&null!==t.query&&null!==t.query.results&&"Yahoo! Weather Error"!==t.query.results.channel.description){
            			...
            			r.heatindex=-42.379+2.04901523*n.item.condition.temp+10.14333127*n.atmosphere.humidity-.22475541*n.item.condition.temp*n.atmosphere.humidity-6.83783*Math.pow(10,-3)*Math.pow(n.item.condition.temp,2)-5.481717*Math.pow(10,-2)*Math.pow(n.atmosphere.humidity,2)+1.22874*Math.pow(10,-3)*Math.pow(n.item.condition.temp,2)*n.atmosphere.humidity+8.5282*Math.pow(10,-4)*n.item.condition.temp*Math.pow(n.atmosphere.humidity,2)-1.99*Math.pow(10,-6)*Math.pow(n.item.condition.temp,2)*Math.pow(n.atmosphere.humidity,2) : 
            			...
            			"3200"==n.item.forecast[m].code?(o.thumbnail=a,o.image=a):(o.thumbnail="https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/"+n.item.forecast[m].code+"ds.png",o.image="https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/"+n.item.forecast[m].code+"d.png"),
            			r.forecast.push(o);
            			i.success(r)
					}
					else i.error("读取雅虎天气数据失败！")
            	},
            	timeout: 5000,
            	error: function (XMLHttpRequest, textStatus, errorThrown) {
            		i.error("访问网址失败，请检查网络连接！");
            		console.log("访问网址失败，请检查网络连接！");
            	}
       	 	});
