...
public class Service : System.Web.Services.WebService
{
    ...
    [WebMethod(Description = "获得当前雨量数据，返回XmlDocument类型数据列表")]
    public XmlDocument getRealtime()
    {
        connectDB();
        if (IsCanConnectioned == true)
        {
            ...
            string sqlcmdreal = string.Format("USE [dcRain_Data] DECLARE	@return_value int EXEC @return_value = [dbo].[XC_HW_AllStation_Realtime_RainList] "+
            ...
            for (int m = 0; m < dsReal.Tables[0].Rows.Count; m++)
            {
                ...
                StationName.InnerText = dsReal.Tables[0].Rows[m][1].ToString();
                item.AppendChild(StationName);
                XmlNode DataTime = doc.CreateElement("DataTime");
                DataTime.InnerText = dsReal.Tables[0].Rows[m][4].ToString();
                item.AppendChild(DataTime);
                XmlNode RainValue = doc.CreateElement("RainValue");
                RainValue.InnerText = dsReal.Tables[0].Rows[m][5].ToString();
    ...
    [WebMethod(Description = "获得两个时间段的雨量数据，返回XmlDocument类型数据列表")]
    public XmlDocument getRainBetweenTwoTime(string StartTime,string EndTime)
    {
        connectDB();
        if (IsCanConnectioned == true)
        {
            ...
            string sqlcmdreal = string.Format("USE [dcRain_Data] DECLARE	@return_value int EXEC	@return_value = [dbo].[XCAllStation_BetweenTwoTime_SumRain] "+
        "@strStartDatetime = N'" + StartTime + "', @strEndDatetime = N'" + EndTime + "' SELECT	'Return Value' = @return_value");
            ...
            for (int m = 0; m < dsReal.Tables[0].Rows.Count; m++)
            {
                ...
                //创建四级节点
                XmlNode StationName = doc.CreateElement("StationName");
                StationName.InnerText = dsReal.Tables[0].Rows[m][3].ToString();
                item.AppendChild(StationName);
                XmlNode RainValue = doc.CreateElement("RainValue");
                RainValue.InnerText = dsReal.Tables[0].Rows[m][1].ToString();
                item.AppendChild(RainValue);
